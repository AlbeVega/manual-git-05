...
4. Gestión Avanzada del Ciclo de Vida
...
git commit
Git Rebase frente a Git Merge
git merge: Crea un merge commit que conserva la historia real y cronológica, incluyendo las ramas paralelas. Es ideal para mantener la trazabilidad de cuándo y cómo se integró una funcionalidad.

git rebase: Reescribe el historial aplicando tus cambios sobre la punta de la rama base, creando una línea de tiempo lineal y limpia.

Nota importante: Nunca realices rebase en ramas públicas o compartidas, ya que altera el historial y puede generar conflictos graves para otros desarrolladores.

Trunk-Based Development
A diferencia de Git Flow, esta metodología busca que los desarrolladores integren cambios pequeños y frecuentes directamente en la rama principal (varias veces al día). Es el estándar actual para equipos que implementan CI/CD (Integración y Despliegue Continuo), ya que reduce al mínimo el riesgo de conflictos complejos a largo plazo.

Manejo de Cambios Temporales (git stash)
Es la herramienta ideal cuando necesitas cambiar de rama de emergencia, pero tienes cambios locales sin confirmar (uncommitted) que aún no están listos para un commit.
...
Limpieza remota: Para eliminar una rama que ya ha sido integrada en el servidor:
